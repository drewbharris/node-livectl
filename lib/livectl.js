...
// send this computer's MIDI to a server

// steps:
// browse for advertised livectl servers
...
LiveCtl.prototype.sendToServers = function(messageName, message){
	var self = this;
	Object.keys(this.servers).map(function(server){
		var msg = new Buffer(JSON.stringify({
			'id': messageName,
			'message': message
		}));
		self.socket.send(msg, 0, msg.length,
			self.servers[server].port, self.servers[server].address, function(err, bytes) {
